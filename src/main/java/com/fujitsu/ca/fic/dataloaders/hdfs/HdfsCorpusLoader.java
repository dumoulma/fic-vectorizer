...
public class HdfsCorpusLoader<E> implements Iterable<E> {
    ...
    public HdfsCorpusLoader(Configuration conf, String inputDirName, LineParser<E> lineParser)
	    throws IOException {
	...
	private List<Path> getListOfMapReduceOutputFiles(FileSystem fs1, String inputDirName)
		throws IOException {
	    FileStatus[] fileStatus = fs1.listStatus(new Path(inputDirName), new PathFilter() {
		@Override
