...
public class BnsCorpusVectorizer implements CorpusVectorizer {
	...
	try {
	    ...
	    writer = SequenceFile.createWriter(FileSystem.get(conf), conf, outputPath, Text.class,
		    VectorWritable.class);

	    Iterable<Vector> corpusLoader = loaderFactory.buildLoader(conf, inputDirName,
		    lineParser);
	    long index = 0L;
